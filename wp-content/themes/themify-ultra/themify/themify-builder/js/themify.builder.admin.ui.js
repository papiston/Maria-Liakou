...
(function ($, window, document, undefined) {
	...
	var builder_api = { Models: {}, Views: {} };

	builder_api.Models.Module = Backbone.Model.extend({
		defaults: {
			mod_name: '',
			mod_settings: {}
		}
	...
	builder_api.Models.Column = Backbone.Model.extend({
		defaults: {
			column_order: '',
			grid_class: '',
			modules: {},
	...
	builder_api.Models.Row = Backbone.Model.extend({
		defaults: {
			row_order: 0,
	...
	builder_api.Views.Module = Backbone.View.extend({
		template: wp.template('builder_module_item'),
		render_template: function( json ){
			var params = { 
				slug: json.mod_name, 
				name: json.mod_name
			};
		...
		render: function() {
			...
			if ( ! _.isUndefined( this.model.get('cols') ) ) {
				var subRowView = new builder_api.Views.SubRow({ model: this.model });
				this.$el.html( subRowView.$el.html() );
			} else {
				this.$el.html( this.render_template( this.model.toJSON() ) );
	...
	builder_api.Views.Column = Backbone.View.extend({
		...
		render: function() {
			...
			if ( ! _.isEmpty( this.model.get('modules') ) ) {
				...
				_.each( this.model.get('modules'), function( value, key ){
					var moduleModel = new builder_api.Models.Module( value ),
						moduleView = new builder_api.Views.Module( { model: moduleModel } );

					moduleLists += moduleView.$el.html();
				});
				this.$el.find('.themify_module_holder').html( moduleLists );
			}
			return this;
		}
	});
	...
	builder_api.Views.SubRow = Backbone.View.extend({
		...
		render: function() {
			...
			if ( ! _.isUndefined( this.model.get('cols') ) ) {
				...
				_.each( this.model.get('cols'), function( value, key ) {
					...
					var columnModel = new builder_api.Models.Column( value ),
						columnView = new builder_api.Views.Column( { model: columnModel } );

					columnLists += columnView.$el.html();
				});
				this.$el.find('.themify_builder_sub_row_content').html( columnLists );
			}
		}
	});

	builder_api.Views.Row = Backbone.View.extend({
		...
		render: function() {
			...
			if ( ! _.isEmpty( this.model.get('cols') ) ) {
				...
				this.$el.find('.themify_builder_row_content').html( columnLists );
			}
		}
	});
	...
	ThemifyPageBuilder = {
		...
		optionsModule: function (e, isNewModule) {
			...
			ThemifyBuilderCommon.Lightbox.open( { loadMethod: 'inline', templateID: 'builder_form_module_' + module_name }, function( response ){
				setTimeout( function(){
					callback( response );
				}, 400);
			});
			...
			switch (component) {
				case 'row':
					...
					var rowDataPlainObject = JSON.parse(dataInJSON),
						rowModel = new builder_api.Models.Row( rowDataPlainObject ),
						rowView = new builder_api.Views.Row( {model: rowModel });

					rowView.$el.children().insertAfter( $selectedRow );
					$selectedRow.remove();
					self.moduleEvents();
				...
				case 'sub-row':
					var $selectedSubRow = $thisElem.closest('.themify_builder_sub_row'),
						...
						subRowModel = new builder_api.Models.Module( subRowDataPlainObject ),
						subRowView = new builder_api.Views.SubRow({model: subRowModel });

					subRowView.$el.children().insertAfter( $selectedSubRow );
				...
				case 'module':
					...
					var moduleModel = new builder_api.Models.Module( modDataPlainObject ),
						moduleView = new builder_api.Views.Module( { model: moduleModel } );

					moduleView.$el.find('.themify_builder_module').insertAfter($selectedModule);
				...
				case 'sub-column':
					var $selectedCol = $thisElem.closest('.themify_builder_col'),
						...
						var columnModel = new builder_api.Models.Column( colDataPlainObject ),
						columnView = new builder_api.Views.Column( { model: columnModel } );

						$selectedCol.html( columnView.$el.find('.themify_builder_col').html() );
						self.moduleEvents();
					break;
			}
		...
		importRowModBuilderFormSave: function(event) {
			...
			switch (component) {
				case 'row':
					...
					rowView.$el.children().insertAfter( $currentSelectedRow );
				...
				case 'module':
					...
					moduleView.$el.find('.themify_builder_module').insertAfter($('.current_selected_module'));
				...
				case 'sub-column':
					...
					$column.html( columnView.$el.find('.themify_builder_col').html() );
